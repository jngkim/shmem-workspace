...
static void print_usage(int myid)
{
    if (myid == 0) {
        if (MEMORY_SELECTION) {
            fprintf(stderr, "Usage: fadd_put_single <heap|global> [call_amo] "
            ...
            fprintf(stderr, "Usage: fadd_put_single [call_amo] "
                            "[random_dest] [min_bytes] [max_bytes]\n");
        }
    }
}
...
void print_operation_rate(int myid, size_t bytes, double rate, double lat)
{
    if (myid == 0) {
        fprintf(stdout, "%-*zu%*s%*.*f%*.*f\n", 20, bytes, FIELD_WIDTH,
                "fadd_put_single", FIELD_WIDTH, FLOAT_PRECISION, rate,
                    ...
                    long *buffer, int print_stats)
{
    ...
    *rate = *sum_rate = *lat = *sum_lat = 0.0;
    ...
    // shmem_int_sum_reduce(SHMEM_TEAM_WORLD, sum_dest_count, dest_count,
    // v.npes); print_statistics(v.me, sum_dest_count, v.npes);
